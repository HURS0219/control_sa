...
static void SendTelemetry(void) {
  ...
  for (int i = 0; i < DART_MOTOR_COUNT; i++) {
    ...
    int mode = (int)m->mode;
    int target100 = (mode == DART_MODE_STOP) ? 0 : (int)(m->target * 100.0f);
    len += snprintf(buf + len, sizeof(buf) - len, ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", i,
                    ...
                    mode, target100, (int)rpm_out, (int)(DartMotorOutAngle(i) * 100.0f),
                    (int)(DartMotorOutTurns(i) * 100.0f), (int)inst->measure.temperature,
