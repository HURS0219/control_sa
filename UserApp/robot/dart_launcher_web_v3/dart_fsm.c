...
static void SpringSetTurns(float turns) {
  DartMotorSetCtrl(DART_ROLE_SPRING_A, DART_MODE_TURNS, turns);
  DartMotorSetCtrl(DART_ROLE_SPRING_B, DART_MODE_TURNS, turns);
}

static int SpringAtTurns(float turns) {
  return DartMotorAtTurns(DART_ROLE_SPRING_A, turns, DART_POS_TOL_TURNS) &&
         DartMotorAtTurns(DART_ROLE_SPRING_B, turns, DART_POS_TOL_TURNS);
}
  ...
  if (g_yaw_mode == DART_YAW_VISION) {
    if (g_vis_ok) {
      ...
      if (fabsf(err) < DART_YAW_AIM_DEADBAND) {
        DartMotorSetCtrl(DART_ROLE_YAW, DART_MODE_SPEED, 0.0f);
      } else {
        float spd = DART_YAW_AIM_KP * err;
        VAL_LIMIT(spd, -YAW_AIM_MAX_RPM, YAW_AIM_MAX_RPM);
        DartMotorSetCtrl(DART_ROLE_YAW, DART_MODE_SPEED, spd);
      }
    } else {
      DartMotorSetCtrl(DART_ROLE_YAW, DART_MODE_SPEED, 0.0f);
    }
  } else if (g_yaw_mode == DART_YAW_GUIDE) {
    DartMotorSetCtrl(DART_ROLE_YAW, DART_MODE_ANGLE, DART_YAW_GUIDE_DEG);
  }
}
